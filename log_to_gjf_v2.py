...
"""
#####################################################
# Specify if you would like to include log files with errors in the conversion
errors = True
#errors = False
#####################################################
...
for i in range(len(l_norms)):
    ...
    for line in lines:
        if 'SCF Done:' in line:
            HF = line.split()[4]
    t_filesort.append((float(HF),l_norms[i]))

## sort energies
t_filesort.sort(key=lambda tup: tup[0])

log_filenames = []
for i in range(len(t_filesort)):
    log_filenames.append(t_filesort[i][1])
